        ...
        <div className="w-full md:w-1/2 lg:w-2/5 md:sticky md:top-4 md:self-start">
          ...
          <div className="aspect-square max-w-sm overflow-hidden rounded-md border border-zinc-700">
            ...
            >
              ...
              <div
                className="h-full w-full bg-no-repeat bg-center bg-contain bg-white"
          ...
          {/* Thumbnail Carousel - Fully clickable */}
              ...
              {imageUrls.map((url: string | undefined) => (
                ...
                onContextMenu={(e) => e.preventDefault()}>
                  <div 
                    ...
                    onClick={() => {
                      const mainImage = document.getElementById("mainImageZoom");
                      if (mainImage && url) {
                        mainImage.style.backgroundImage = `url(${url})`;
                      }
                    }}
                  >
                    {/* Thumbnail as protected background image */}
                    <div 
                      className="w-full h-full bg-no-repeat bg-center bg-cover bg-white rounded-md"
        ...
        <AccordionItem value="spec-details">
          ...
          <AccordionContent>
            <div className="overflow-x-auto">
              <table className="w-full rounded-lg border bg-slate-200 drop-shadow-sm dark:bg-slate-800">
                ...
                <tbody>
                  {componentInfo?.map((info: any, index) => (
                    ...
                    >
                      <td className="whitespace-nowrap border-r border-slate-700 p-3 sm:p-4 text-slate-300 text-sm sm:text-base">
                        {info.title}
                      </td>
                      <td className="w-full p-3 sm:p-4 text-slate-100 text-sm sm:text-base">
                        {(() => {
                          ...
                          if (typeof info.value === 'object') {
                            return <pre className="text-xs sm:text-sm bg-slate-800/50 p-2 rounded overflow-x-auto">{JSON.stringify(info.value, null, 2)}</pre>;
                          }
                          ...
                          return <span className="break-all">{info.value.toString()}</span>;
                        })()}
                      </td>
                    </tr>
                  ))}
