...
export default component$<JasaProps>((props) => {
  ...
  const arrow = (
    <footer
      ...
      onClick$={() =>
        props.labelFor || props.href ? undefined : alert('Coming Soon!')
      }
    >
  ...
  return (
    <div class="h-full py-8">
      <div
        class={`min-w-fit max-w-sm px-4 bg-white rounded-lg shadow-xl shadow-slate-200 h-full ${borderClass}`}
      >
        <header class={[style['jasa-header'], 'mx-auto translate-y-[-50%]']}>
          <div
            class={[
              style['jasa-icon'],
              style[`icon${props.iconNumber}`],
              'mx-auto rounded-full justify-center items-center aspect-square w-14',
            ]}
          >
            <Slot name="icon" />
          </div>
        ...
        >
          ...
          <span class="m-auto">{props.description}</span>
        </main>
      </div>
      {props.labelFor && <label for={props.labelFor}>{arrow}</label>}
      {props.href && (
