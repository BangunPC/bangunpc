...
import {
  CatalogueSidebar,
  SidebarSection,
} from '~/components/catalogue/sidebar';
...
export default component$(() => {
  ...
  const filters = useStore({
    minPrice:
      url.searchParams.get('min-price') === '0'
        ? undefined
        : url.searchParams.get('min-price'),
    maxPrice:
      url.searchParams.get('max-price') === '0'
        ? undefined
  ...
  const components = useResource$(async ({ track }) => {
    ...
    return await getPsu(
      {
        cpuId: parseInt(
          localComponents.value.find(
            (c) => c.category === ComponentCategory.CPU
          )?.id ?? ''
        ),
        // TODO(katalog): multiple gpu
        gpuId: parseInt(
          localComponents.value.find(
            (c) => c.category === ComponentCategory.GPU
          )?.id ?? ''
        ),
        memories: localComponents.value
          .filter((c) => c.category === ComponentCategory.Memory)
          .map((c) => ({ id: parseInt(c.id), amount: c.quantity })),
        motherboardId: parseInt(
          localComponents.value.find(
            (c) => c.category === ComponentCategory.Motherboard
          )?.id ?? ''
        ),
        storages: localComponents.value
          .filter((c) => c.category === ComponentCategory.Storage)
          .map((c) => ({ id: parseInt(c.id), amount: c.quantity })),
      },
      {
