...
export default component$(() => {
  ...
  const refresh = $(function refresh() {
    ...
    cooler.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Cooler
    );
    motherboard.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Motherboard
    );
    memory.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Memory
    );
    storage.value = ComponentStorage.getComponentsByCategory(
    ...
    casing.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Casing
    );
    caseFan.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.CaseFan
    );
    monitor.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Monitor
    );
    os.value = ComponentStorage.getComponentsByCategory(ComponentCategory.OS);
    soundCard.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.SoundCard
    );
    wiredNetwork.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.WiredNetwork
    );
    wirelessNetwork.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.WirelessNetwork
    );
    cable.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Cable
    );
    externalDrive.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.ExternalDrive
    );
    headphone.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Headphone
    );
    keyboard.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Keyboard
    );
    mouse.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Mouse
    );
    speaker.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Speaker
    );
    webcam.value = ComponentStorage.getComponentsByCategory(
      ComponentCategory.Webcam
    );
    printer.value = ComponentStorage.getComponentsByCategory(
  ...
  const components = [
    {
      icon: <SimulasiMotherboard width="27" height="27" />,
      title: 'Motherboard',
      components: motherboard,
      iframe: `/katalog/motherboard`,
    },
    {
      icon: <SimulasiCpu width="27" height="27" />,
      title: 'CPU',
      components: cpu,
      iframe: `/katalog/cpu`,
    },
    {
      icon: <SimulasiGpu width="27" height="27" />,
      title: 'GPU',
      components: gpu,
      iframe: `/katalog/gpu`,
    },
    {
      icon: <SimulasiMemory width="27" height="27" />,
      title: 'Memory',
      components: memory,
      iframe: `/katalog/memory`,
    },
    {
      icon: <SimulasiPsu width="27" height="27" />,
      title: 'Power Supply',
      components: psu,
      iframe: `/katalog/psu`,
    },
    {
      icon: <SimulasiStorage width="27" height="27" />,
      title: 'Storage',
      components: storage,
      iframe: `/katalog/storage`,
    },
    {
      icon: <SimulasiCasing width="27" height="27" />,
      title: 'PC Case',
      components: casing,
  ...
  const handleAddComponent = $(function handleAddComponent(
    ...
    switch (item.title) {
      case 'CPU':
        ...
        if (memory.value.length > 0) {
          query.memories = memory.value.map((item) => ({
            id: parseInt(item.id),
    ...
    convertToUrlQuery(query).then((result) => {
      urlQuery.value = result;
      iframePath.value = item.iframe;
    });
  });
  return (
    <div class="mt-1 p-4 max-w-screen-desktop w-full m-auto">
      ...
      <main class="flex flex-col gap-4 p-4 max-w-screen-desktop w-full m-auto">
            ...
            <tbody>
              {components.map((item) => {
                const bottomSpace = (item.title == 'Memory' ||
                  item.title == 'Storage') && <div class="h-[38px]" />;
                return (
        ...
        <div
          role="dialog"
          aria-modal="true"
          class={`fixed mt-navbar-min-h z-10 inset-0 overflow-y-hidden ${
            iframePath.value === '' ||
            currentIframePath.value.startsWith('/simulasi/') ||
            currentIframePath.value === '/'
              ? 'hidden'
              : 'block'
          }`}
            ...
            <div class="flex flex-col align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all h-full">
              ...
              <div class="flex justify-end bg-white">
                <FilledButton
                  ...
                  class="aspect-square fixed ml-auto mt-4 mr-4"
                >
                  <TbX />
                </FilledButton>
              </div>
              <iframe
                ...
                src={
                  urlQuery.value
                    ? iframePath.value + `?${urlQuery.value}&iframe=true`
                    : iframePath.value + '?iframe=true'
                }
              />
            </div>
          </div>
